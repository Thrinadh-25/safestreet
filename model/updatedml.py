...
# road_model.load_state_dict(torch.load(r"C:\safestreetdupe\ml_server\road_classifier.pth", map_location=device))
# road_model.eval()
...
# damage_model.load_state_dict(torch.load(r"C:\safestreetdupe\ml_server\best_damage_vit_model (1).pth", map_location=device))
# damage_model.eval()
...
damage_model = DamageViTModel(num_classes=NUM_CLASSES, num_severity=NUM_SEVERITY).to(device)
damage_model.load_state_dict(torch.load(r"C:\safestreet\model\best_damage_vit_model (1).pth", map_location=device))
damage_model.eval()
