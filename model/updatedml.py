...
# === LABELS ===
CLASS_LABELS = ["Longitudinal Crack", "Transverse Crack", "Alligator Crack", "pothole"]
SEVERITY_LABELS = ["Low", "Medium", "High"]
...
# === Road Check Model (SmallCNN) ===
class RoadCheckModel(nn.Module):
    def __init__(self, num_classes=2):  # Fixed typo: _init_ → __init__
        super(RoadCheckModel, self).__init__()
        self.features = nn.Sequential(
        ...
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(64 * 28 * 28, 128),  # Assuming input size 224x224, after 3 maxpool layers: 224/8 = 28
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(128, num_classes)
        )
...
# === ViT Damage Model ===
class DamageViTModel(nn.Module):
    def __init__(self, num_classes, num_severity):  # Fixed typo: _init_ → __init__
    ...
    def forward(self, pixel_values):
        ...
        bbox_pred = torch.sigmoid(self.bbox_regressor(cls_token))
        severity_logits = self.severity_classifier(cls_token)
        return {'class_logits': outputs.logits, 'bbox_pred': bbox_pred, 'severity_logits': severity_logits}

# --- Model Loading ---
print("--- Loading models... ---")
basedir = os.path.abspath(os.path.dirname(__file__))
road_model_path = os.path.join(basedir, 'road_classifier.pth')
damage_model_path = os.path.join(basedir, 'best_damage_vit_model (2).pth')

road_model = RoadCheckModel().to(device)
road_model.load_state_dict(torch.load(road_model_path, map_location=device))
road_model.eval()
...
def process_request(image_file):
    ...
    with torch.no_grad():
        ...
        road_pred = torch.argmax(road_output, dim=1).item()
    if road_pred == 0:
        return {"error": "Image is not a road photo. Please upload a road image."}
...
def analyze_json():
    ...
    result = process_request(request.files['image'])
    if "error" in result:
        return jsonify(result), 400

    w_img, h_img = result['image'].size
    predicted_class = result['output']['class_logits'].argmax(dim=1).item()
    severity_class = result['output']['severity_logits'].argmax(dim=1).item()
    bbox = result['output']['bbox_pred'][0].tolist()
    
    cx, cy, bw, bh = bbox
    bw = min(bw, 0.25)
    bh = min(bh, 0.25)
    x1, y1, x2, y2 = int((cx - bw/2) * w_img), int((cy - bh/2) * h_img), int((cx + bw/2) * w_img), int((cy + bh/2) * h_img)
    
    class_label = CLASS_LABELS[predicted_class]
    severity_label = SEVERITY_LABELS[severity_class]
    summary = f"A road damage of type '{class_label}' was detected with '{severity_label}' severity."

    return jsonify({"type": class_label, "severity": severity_label, "summary": summary, "bbox_pixels": [x1, y1, x2, y2]})
...
def analyze_annotate():
    ...
    draw.text((x1, max(0, y1 - 35)), f"{class_label} ({severity_label})", fill="white", font=font, stroke_width=2, stroke_fill="red")
